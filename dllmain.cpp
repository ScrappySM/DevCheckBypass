...
static constexpr size_t PATCH_SIZE = 6;

// Buffer to store original bytes
std::array<uint8_t, PATCH_SIZE> originalBytes;
...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD dwReason, LPVOID lpReserved) {
    static uintptr_t pAddress = reinterpret_cast<uintptr_t>(GetModuleHandle(nullptr)) + pOffset;
    ...
    if (dwReason == DLL_PROCESS_ATTACH) {
        ...
        // Save original bytes
        memcpy(originalBytes.data(), reinterpret_cast<void*>(pAddress), PATCH_SIZE);

        // Change memory protection
        DWORD oldProtect;
        if (!VirtualProtect(reinterpret_cast<void*>(pAddress), PATCH_SIZE, PAGE_EXECUTE_READWRITE, &oldProtect)) {
        ...
        memset(reinterpret_cast<void*>(pAddress), 0x90, PATCH_SIZE);

        // Restore protection
        VirtualProtect(reinterpret_cast<void*>(pAddress), PATCH_SIZE, oldProtect, &oldProtect);
    }
    else if (dwReason == DLL_PROCESS_DETACH) {
        // Restore original instruction
        DWORD oldProtect;
        if (VirtualProtect(reinterpret_cast<void*>(pAddress), PATCH_SIZE, PAGE_EXECUTE_READWRITE, &oldProtect)) {
            memcpy(reinterpret_cast<void*>(pAddress), originalBytes.data(), PATCH_SIZE);
            VirtualProtect(reinterpret_cast<void*>(pAddress), PATCH_SIZE, oldProtect, &oldProtect);
        }
    }
